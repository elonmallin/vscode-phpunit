...
const getOnDidEndTaskProcessPromise = (
  outputFile: string,
  ...
  return new Promise<{ output: string; exitCode: number }>(
    (resolve, reject) => {
      const disposable = vscode.tasks.onDidEndTaskProcess(async (e) => {
        if (
          e.execution.task.source === "phpunit" &&
          e.execution.task.name === "run"
        ) {
          ...
          resolve({
            output,
            exitCode: e.exitCode === undefined ? 1 : e.exitCode,
          });
        }
      });
    },
...
suite("php-project e2e", () => {
  ...
  test("phpunit.TestDirectory2", async () => {
    const uri = vscode.Uri.file(
      path.resolve(
        vscode.workspace.workspaceFolders![0].uri.fsPath,
        "tests/Science/Weather-1/Weather2Test.php",
      ),
    );
    const document = await vscode.workspace.openTextDocument(uri);
    await vscode.window.showTextDocument(document);
  ...
  test("phpunit.TestDirectory3", async () => {
    const uri = vscode.Uri.file(
      path.resolve(
        vscode.workspace.workspaceFolders![0].uri.fsPath,
        "tests/Science2/Weather-1/Weather/Weather3Test.php",
      ),
    );
    const document = await vscode.workspace.openTextDocument(uri);
    await vscode.window.showTextDocument(document);
