...
async function getOrCreateFromTestCaseNodes(
  controller: TestController,
  uri: Uri,
  testCaseNodes: Array<TestCaseNode>,
  parentItemCollecton: TestItemCollection,
): Promise<TestItem[]> {
  for (const testCaseNode of testCaseNodes) {
    if (testCaseNode.kind === "namespace") {
      if (testCaseNode.children?.length > 0) {
        await getOrCreateFromTestCaseNodes(
          controller,
          uri,
      ...
      const testCase = new TestClass(
        uri.fsPath,
        testCaseNode.name,
        testCaseNode.range,
        testCaseNode.parent?.name,
      );
      ...
      const testCase = new TestMethod(
        uri.fsPath,
        testCaseNode.name,
        testCaseNode.range,
        testCaseNode.parent?.name,
        testCaseNode.parent?.parent?.name,
      );
      let testItem = parentItemCollecton.get(testCase.getId());
      if (!testItem) {
        testItem = controller.createTestItem(
...
export async function deleteFromUri(
  controller: TestController,
...
export async function createOrUpdateFromPath(
  controller: TestController,
  filePath: string,
